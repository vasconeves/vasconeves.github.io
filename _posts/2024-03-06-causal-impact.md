...
In this project I use Causal Impact Analysis to analyse & understand the sales uplift of customers that joined the new "Delivery Club" campaign of the well-known retail store that I've been working for some time.
...
The *comparable data* that we pass in can be a control group, another set of related data, or even multiple sets of related data - but for this approach to work robustly & reliably, this additional data must not be affected by the event that we’re measuring, but it must be predictive of our output, or have some relationship with our initial time-series data.
...
* Manoeuver the data specifically for the `pycausalimpact` algorithm.
...
Top image sourced at | [https://commons.wikimedia.org/wiki/File:Newtons_Cradle.jpg](https://commons.wikimedia.org/wiki/File:Newtons_Cradle.jpg)